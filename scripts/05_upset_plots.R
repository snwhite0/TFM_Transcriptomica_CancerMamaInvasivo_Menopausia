...
# 6.3. Ejecutar la función para cada objetivo
...
# 6.4. Generar Upset Plots individuales para cada subtipo (objetivo 2)
target_subtypes = c("Basal", "Her2","LumA", "LumB")

for (subtipo in target_subtypes) {
  cat("Upset Plot: DEGs Compartidos y Específicos en el subtipo", subtipo)
  
  # Filtrar listas por subtipo
  listas_subtipo = ids_genes_significativos_obj2[grep(paste0("^", subtipo, "_"), names(ids_genes_significativos_obj2))]
  
  # Eliminar listas vacías
  listas_subtipo = listas_subtipo[sapply(listas_subtipo, length) > 0]
...
cat("¡Listo! Todos los UpSet Plots han sido generados y guardados en:\n", upset_plots_dir, "\n")
